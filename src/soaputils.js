...
let xmlExact = require('xml-exact');
...
class SoapUtils {
    constructor(wsdlString, url) {
        ...
        const wsdlObj = xmlExact.fromXml(wsdlString, wsdlDefinition, { inlineAttributes: true });
        ...
        this._soapDefinition.Envelope.Body = xmlExact.generateDefinition(wsdlObj.definitions.types.schema[0], "xsd", namespaces); // TODO: Support more than one schema
        ...
        this._xsdSchema = xmlExact.toXml(schemaObj, "schema", { schema$attributes: namespaces }, { convertTypes: false });
    }

    static fromUrl(wsdlUrl) {
        return _httpGetString(wsdlUrl)
            .then((wsdlString) => {
    ...
    wsdlObj.service.forEach(function(service){
        ...
        service.port.forEach(function(port){
            ...
            portType.operation.forEach(function(portTypeOperation) {
                ...
                let bindingOperation = binding.operation
                ...
                let inputMessageName = portTypeOperation.input.$message.replace(/^[^:]+:/, ""); // Remove namespace
                ...
                let outputMessageName = portTypeOperation.output.$message.replace(/^[^:]+:/, ""); // Remove namespace
