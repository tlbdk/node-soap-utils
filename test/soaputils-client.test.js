...
describe('SOAPClient', function () {
    ...
    it('Create new client and class service', (done) => {
        const client = new SoapUtils(sampleWSDL, "http://localhost:" + port + "/test1");
        let requestSample = client.generateSampleRequest("StockQuoteService", "StockQuoteBinding", "GetLastTradePrice");
        requestSample.TradePriceRequest.tickerSymbol = "GOOG";
        client.sendRequest("StockQuoteService", "StockQuoteBinding", "GetLastTradePrice", requestSample)
            .then(response => {
    ...
    it('Create new client from url', (done) => {
        SoapUtils.fromUrl("http://localhost:" + port + "/StockQuote.wsdl").then((soapUtils) => {
            try {
